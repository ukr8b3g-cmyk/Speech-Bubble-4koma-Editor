...
assert.match(html, /pageRecords=records\.filter\(record=>!singleRecords\.includes\(record\)\)/);
assert.match(html, /sharedPageImageStore\.importRecords\(pageRecords/);
assert.match(html, /sharedPageImageStore\?await sharedPageImageStore\.exportRecords/);
assert.match(generalEditor, /syncSharedImages/);
assert.match(generalEditor, /removeAssetUsage/);
assert.match(generalEditor, /usedImageIds/);
