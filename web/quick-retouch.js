(function (root) {
  ...
  function create(options = {}) {
    ...
    dialog.innerHTML = `
      <div class="quick-retouch-window">
        <header class="quick-retouch-head" data-retouch-drag-handle>
          ...
          <span data-retouch-mode></span><span data-retouch-document></span>
    ...
    async function setSource(next) {
      if (!next?.blob || applying || !dialog.open) return false;
    ...
    sourceDrop?.addEventListener("drop", async (event) => {
      ...
      await setSource({ blob: file, name: file.name, source_kind: "external-file" });
    });
