(function (root) {
  ...
  function create(options = {}) {
    ...
    // Session identity prevents late image decodes from leaking into another document.
    ...
    async function showSourcePicker(forceOpen = false) {
      ...
      for (const candidate of candidates) {
        ...
        button.onclick = async (event) => { event.stopPropagation(); await loadSource(candidate); };
        sourceCandidatesHost.append(button);
      }
      if (!sourceCandidatesHost.children.length) {
        const empty = document.createElement("p");
        empty.textContent = isPageImageMode()
          ? tr("ページ画像から選ぶか、画像ファイルを読み込んでください。", "Choose a Page Image, or load an image file.")
    ...
    async function applyResult() {
      ...
      try {
        ...
        if (session !== sessionGeneration || !dialog.open) return;
    ...
    dialog.addEventListener("drop", event => event.stopPropagation());
    ...
    fileInput.onchange = async () => {
      ...
      fileInput.value = "";
      if (file) await loadSource({ blob: file, name: file.name, source_kind: "external-file" });
    };
    ...
    sourceDrop?.addEventListener("drop", async (event) => {
      ...
      await loadSource({ blob: file, name: file.name, source_kind: "external-file" });
    });
