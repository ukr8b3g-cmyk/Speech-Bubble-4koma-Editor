  ...
  function create(options = {}) {
    ...
    function syncUi() {
      ...
      if (structural && elements.properties) {
        ...
        for (const input of elements.properties.querySelectorAll("[data-general-page]")) { const key = input.dataset.generalPage; const value = key === "template_id" ? comic.template_id : comic.page[key]; if (input.type === "checkbox") input.checked = Boolean(value); else if (document.activeElement !== input) input.value = value; }
    ...
    function handlePropertyChange(event) { const input = event.target.closest("[data-general-page],[data-general-canvas],[data-general-image],[data-general-divider-ratio]"); if (!input) return; if (input.dataset.generalPage === "template_id") { applyTemplate(input.value); return; } delete input.dataset.generalEditing; changed(); syncUi(); }
