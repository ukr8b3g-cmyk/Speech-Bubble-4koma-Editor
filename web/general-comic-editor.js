(function (root) {
  ...
  function create(options = {}) {
    ...
    async function importFiles(files) { const valid = files.filter(supportedImage); if (!valid.length) { options.setStatus?.(tr("PNG / JPEG / WebPを選択してください", "Choose PNG, JPEG, or WebP"), "error"); return []; } if (comic.images.length + valid.length > MAX_IMAGES) { options.setStatus?.(tr("ページ画像は100枚までです", "Up to 100 page images are supported"), "error"); return []; } const ids = []; for (const file of valid) { if (file.size > MAX_IMAGE_BYTES) { options.setStatus?.(tr("画像が大きすぎます", "The image is too large"), "error"); continue; } const metadata = { id: `${IMAGE_PREFIX}${uuid()}`, name: String(file.name || "image").replace(/\.[^.]+$/, ""), mime: file.type || "image/png", width: 1, height: 1, sha256: "", source: "stored" }; await attachBlob(metadata, file); comic.images.push(metadata); await imageStore.put(metadata, file); ids.push(metadata.id); } if (ids.length) { used = true; selectedTrayImageId = ids.at(-1); changed(); renderTray(); } return ids; }
    ...
    async function importProjectImages(records) { for (const record of Array.isArray(records) ? records : []) { if (!record?.id || !String(record.data_url || "").startsWith("data:image/")) continue; const metadata = comic.images.find((item) => item.id === record.id); if (!metadata) continue; const blob = await fetch(record.data_url).then((response) => response.blob()); await attachBlob(metadata, blob); await imageStore.put(metadata, blob); } renderTray(); requestRender({ canvas: true, layers: true }); }
    async function importExternalImage(file, asset, control = {}) { if (!(file instanceof Blob) || !asset?.id) return ""; const metadata = { id: String(asset.id), name: String(asset.name || "page-image").slice(0, 260), mime: asset.mime || file.type || "image/png", width: Math.max(1, Number(asset.width) || 1), height: Math.max(1, Number(asset.height) || 1), sha256: String(asset.sha256 || ""), source: "shared-page-image" }; options.pushUndo?.(); let existing = comic.images.find((item) => item.id === metadata.id); if (!existing) { existing = metadata; comic.images.push(existing); } if (!runtimeImages.has(existing.id)) await attachBlob(existing, file); await imageStore.put(existing, file); selectedTrayImageId = existing.id; const dropped = control.point ? core.panelAtPoint(layout(), control.point)?.node : null; const target = dropped || (control.assignToSelectedPanel ? selectedPanel() : null); if (target?.kind === "panel" && panelEditable(target)) { target.image_id = existing.id; target.image_scale = 1; target.image_offset_x = 0; target.image_offset_y = 0; target.image_visible = true; selectedImagePanelIds.clear(); selectedImagePanelIds.add(target.id); selection = { kind: "image", id: target.id }; lastPanelId = target.id; } used = true; changed(); syncUi(); return existing.id; }
    function removeAssetUsage(imageId, control = {}) { const id = String(imageId || ""); if (!id) return false; const usedPanels = core.collectPanels(comic.tree).filter((panel) => panel.image_id === id); const hasMetadata = comic.images.some((item) => item.id === id); if (!usedPanels.length && !hasMetadata) return false; if (control.recordUndo !== false) options.pushUndo?.(); usedPanels.forEach((panel) => { panel.image_id = null; selectedImagePanelIds.delete(panel.id); }); comic.images = comic.images.filter((item) => item.id !== id); if (selectedTrayImageId === id) selectedTrayImageId = ""; if (selection.kind === "image" && !core.findNode(comic.tree, selection.id)?.image_id) selection = { kind: "panel", id: selection.id }; changed(); syncUi(); return true; }
